...
- **1B.2 Lista de municípios por circunscrição.** 🔜 Curadoria manual em ~280 páginas — ver confirmação de cobertura abaixo. **Piloto das 26 capitais estaduais concluído (2026-09-18)**: 24 com `granularidade_territorial: municipio` (468 municípios cobertos ao todo), 2 sub-municipais genuínas (Diocese de Macapá — cobre todo o AP + só "comunidades de ilhas" em 3 municípios do PA; Arquidiocese de São Paulo — cobre só as regiões episcopais centrais da capital, o resto da cidade e da região metropolitana é de dioceses sufragâneas distintas). Confiança: 16 `alta` (fonte única e explícita, ou duas fontes concordantes), 9 `media` (inferência via agregação GCatholic de igrejas-por-cidade, ou lista parcial), 1 `baixa` (Teresina: Wikipédia declara 22 municípios sem nomear, GCatholic lista 31 nomes dos quais 28 batem com IBGE — discrepância não reconciliada). Roraima (Boa Vista) ficou de fora — é prelazia territorial, ainda deferida (fora do escopo 1-A). Processo por circunscrição: Wikipédia (seção "Território" ou "Demografia e paróquias") como primeira fonte; se ausente, GCatholic `churches/local/<gcatholic_id>` como fallback; todo nome cruzado contra `tools/ibge-municipios.json` normalizando acentos — erros de grafia da fonte (ex. "Manuel"→"Manoel" Urbano, "Jarguari"→Jaraguari, "ltauçu"→Itauçu) corrigidos manualmente, e nomes que não batem (bairros/distritos como "Ponta Negra" em Natal ou "Coxipó" em Cuiabá) descartados da lista, nunca adivinhados. Este processo — não um script — é o que precisa se repetir para as ~254 circunscrições restantes.
