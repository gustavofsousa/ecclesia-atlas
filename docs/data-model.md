...
2. **Nem toda circunscrição tem território.** Ordinariado Militar e Administração Apostólica Pessoal são pessoais, não territoriais. `territorio` é nullable, controlado por `tipo_jurisdicao: territorial | pessoal`. O schema cobre o caso desde a Fase 0; o Ordinariado Militar do Brasil está deferido para uma fase futura (ver `tools/wikidata-deferred.csv` e o ROADMAP), então a v0 ainda não tem um registro `pessoal` real — só o caso `territorial` (ex.: `data/circunscricoes/arquidiocese-de-sao-salvador-da-bahia.json`).
